...
select upper(DevName)  as DevName  ,len( Email) as Email from   Developers;
SELECT DevName,  DATEDIFF(MONTH, JoinDate, GETDATE()) AS total_month
FROM Developers
WHERE DATEDIFF(MONTH, JoinDate, GETDATE()) > 24;
--3
 SELECT TOP 4 DevName,DEpartment,BaseSalary 
FROM Developers  Order by BaseSalary DESC  ; 


--4
SELECT Project.ProjectName, SUM(ProjectAllocations.AllocHours) AS TotalHours 
FROM Project 
JOIN ProjectAllocations  ON Project.ProjectID = ProjectAllocations.ProjectID 
GROUP BY Project.ProjectName HAving  SUM(ProjectAllocations.AllocHours) > 150 ORDER BY Project.ProjectName DESC  ;
